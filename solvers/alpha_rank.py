...
def _get_singlepop_2player_fitness(
    payoff_table,
    ...
    if use_local_selection_model:
        ...
        fitness = (my_popsize - 1) / (m - 1) * _get_payoff(
            payoff_table,
            payoffs_are_hpt_format,
            strat_profile=[my_strat, my_strat],
            k=0,
        ) + (m - my_popsize) / (m - 1) * _get_payoff(
            payoff_table,
            payoffs_are_hpt_format,
            strat_profile=[my_strat, opponent_strat],
            k=0,
        )
    return fitness
...
def _get_rho_sr_multipop(
    payoff_table_k, payoffs_are_hpt_format, k, m, r, s, alpha, use_fast_compute=True
):
...
def _get_singlepop_transition_matrix(
    payoff_table,
    payoffs_are_hpt_format,
    m,
    alpha,
...
def get_multipop_transition_matrix(
    payoff_tables,
    payoffs_are_hpt_format,
    m,
    alpha,
    ...
    return _get_multipop_transition_matrix(
        payoff_tables,
        payoffs_are_hpt_format,
        m,
        alpha,
...
def _get_multipop_transition_matrix(
    payoff_tables,
    payoffs_are_hpt_format,
    m,
    alpha,
    ...
    for id_row_profile in range(num_profiles):
        ...
        for index_population_that_changed, col_profile in next_profile_gen:
            ...
            if use_inf_alpha:
                ...
                c[id_row_profile, id_col_profile] = (
                    eta * rhos[id_row_profile, id_col_profile]
                )
...
def _get_stationary_distr(c):
    ...
    if num_stationary_eigenvecs != 1:
        raise ValueError(
            "Expected 1 stationary distribution, but found %d"
...
def print_results(
    payoff_tables, payoffs_are_hpt_format, rhos=None, rho_m=None, c=None, pi=None
):
    """Prints the finite-population analysis results_109."""
...
def sweep_pi_vs_epsilon(
    ...
    Args:
      ...
      visualize: Plot the sweep results_109.
    ...
    rhos_list, rhom_list, c_list = {}, {}, {}
    while True:
        try:
            pi_prev = pi
            rhos, rhom, c, pi, _, _ = compute(
                payoff_tables,
                m=m,
                alpha=alpha,
                use_inf_alpha=True,
            ...
            rhos_list[epsilon] = rhos
            rhom_list[epsilon] = rhom
            c_list[epsilon] = c
            ...
            assert epsilon >= min_epsilon, (
                "AlphaRank stationary distr. not found &" "epsilon < min_epsilon."
            )
    ...
    if return_epsilon:
        return (
            pi_list[-1],
            epsilon_list[-1],
            rhos_list[epsilon_list[-1]],
            rhom_list[epsilon_list[-1]],
            c_list[epsilon_list[-1]],
        )
    else:
        return (
            pi_list[-1],
...
def sweep_pi_vs_alpha(
    ...
    while 1:
        try:
            rhos, rhom, c, pi, _, _ = compute(payoff_tables, alpha=alpha, m=m)
            rhos_list[alpha] = rhos
            rhom_list[alpha] = rhom
            c_list[alpha] = c
    ...
    if return_alpha:
        return pi, alpha, rhos_list[alpha], rhom_list[alpha], c_list[alpha]
    else:
        return pi, rhos_list[alpha], rhom_list[alpha], c_list[alpha]


def compute_and_report_alpharank(
    payoff_tables, m=50, alpha=100, verbose=False, num_top_strats_to_print=8
):
    ...
    Args:
      ...
      alpha: Fermi distribution temperature parameter.
      verbose: Set to True to print intermediate results_109.
...
def compute(
    ...
    if np.array_equal(
        num_strats_per_population, np.ones(len(num_strats_per_population))
    ):
        rhos = np.asarray([[1]])
    ...
    rho_m = 1.0 / m if not use_inf_alpha else 1  # Neutral fixation probability
    if verbose:
        print_results(payoff_tables, payoffs_are_hpt_format, rhos, rho_m, c, pi)

    return rhos, rho_m, c, pi, num_profiles, num_strats_per_population
...
def remove_epsilon_negative_probs(probs, epsilon=1e-6):
...
def alpharank_strategy(meta_game, return_joint=False, **unused_kwargs):
    ...
    joint_distr, _, _, _ = sweep_pi_vs_epsilon(meta_games)
    # print(joint_distr)
    # if len(joint_distr[joint_distr < 0]) > 0:
    #     if not np.alltrue(np.min(joint_distr[joint_distr < 0]) > -1.0 * 1e-8):
    #         print(meta_games)
    joint_distr = remove_epsilon_negative_probs(joint_distr, epsilon=1.0)
