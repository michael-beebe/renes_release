...
class RolloutStorage(object):
    ...
    def insert(
        ...
        self.obs_general.append(deepcopy(obs))
    ...
    def after_update(self):
        ...
        temp_obs = deepcopy(self.obs_general[-1])
        self.obs_general.clear()
        self.obs_general.append(temp_obs)
    ...
    def feed_forward_generator(
        ...
        temp_obs_general = []
        for step in range(len(self.obs_general) - 1):
            for p in range(num_processes):
                temp_obs_general.append(self.obs_general[step][p])
        for indices in sampler:
            # obs_batch = self.obs[:-1].view(-1, *self.obs.size()[2:])[indices]
            # print("len indices: {}".format(len(indices)))
            obs_batch = {"gnn_original": [], "gnn_current": []}

            for idx in indices:
                obs_batch["gnn_original"].append(
                    deepcopy(temp_obs_general[idx]["gnn"][0]).to(self.device)
                )
                obs_batch["gnn_current"].append(
                    deepcopy(temp_obs_general[idx]["gnn"][1]).to(self.device)
                )
