    ...
    def __init__(self, num_in_feats, num_out_feats, num_layers=3, batch_norm=True):
        ...
        for l in range(self.num_layers):
            if l == 0:
                ...
                if batch_norm:
                    self.sequential.append(nn.BatchNorm1d(self.num_out_feats))
                self.sequential.append(nn.ReLU())
            elif l == self.num_layers - 1:
                self.last_linear = nn.Linear(self.num_out_feats, self.num_out_feats)
                if batch_norm:
                    self.output_seq.append(nn.BatchNorm1d(self.num_out_feats))
            else:
                self.sequential.append(
                    nn.Linear(self.num_out_feats, self.num_out_feats)
                )
                if batch_norm:
                    self.sequential.append(nn.BatchNorm1d(self.num_out_feats))
                self.sequential.append(nn.ReLU())
...
class GCN(nn.Module):
    ...
    def forward(self, graph_batch):
        ...
        # else:
        #     raise ValueError(
        #         "Unknown combination of data types: {}".format(
        #             ",".join([type(x) for x in args])
        #         )
        #     )
...
class GraphAttentionPooling(nn.Module):
    ...
    def setup_weights(self):
        ...
        self.weight_matrix = torch.nn.Parameter(
            torch.Tensor(self.feat_dim, self.feat_dim)
        )
...
class AttentionModule(nn.Module):
    ...
    def __init__(self, feat_dim_query, feat_dim_val):
        ...
        self.weight_query = nn.Parameter(
            torch.Tensor(self.feat_dim_query, self.hidden_dim)
        )
...
class TensorNetworkModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        self.weight_matrix_block = torch.nn.Parameter(
            torch.Tensor(self.tensor_neurons, 2 * self.input_features)
        )
    ...
    def forward(self, embedding_1, embedding_2):
        ...
        scoring = torch.matmul(
            embedding_1, self.weight_matrix.view(self.input_features, -1)
        )
        scoring = scoring.view(batch_size, self.input_features, -1).permute([0, 2, 1])
        scoring = torch.matmul(
            scoring, embedding_2.view(batch_size, self.input_features, 1)
        ).view(batch_size, -1)
